...
report="artifacts/security-review.md"
sarif_report="artifacts/security-review.sarif"
...
if [[ -z "${php_files}" ]]; then
  echo '{"active_findings":[]}' | php scripts/ai-gates/analyzers/findings_to_sarif.php --gate security_review > "$sarif_report"
  write_report "$report" "PASS" "Laravel Security Review" <<'REPORT'
## Findings
...
filtered_output="$(php scripts/ai-gates/analyzers/filter_findings.php --gate security_review --policy "$policy_file" --baseline "$baseline_file" <<< "$json_output")"
php scripts/ai-gates/analyzers/findings_to_sarif.php --gate security_review <<< "$filtered_output" > "$sarif_report"
...
fail_on="$(php -r '$p=json_decode(file_get_contents($argv[1]),true); $v=$p["thresholds"]["security_review"]["fail_on"] ?? ["high"]; echo json_encode($v);' "$policy_file")"
...
if [[ "$finding_count" -gt 0 ]]; then
  ...
  should_fail="$(php -r '
  $fail=json_decode($argv[1], true) ?: ["high"];
  $r=json_decode(stream_get_contents(STDIN), true);
  ...
  write_report "$report" "$status" "Laravel Security Review" <<REPORT
## Findings
${findings_md}
...
write_report "$report" "PASS" "Laravel Security Review" <<REPORT
## Findings
- AST analysis found no high-confidence security anti-patterns in changed PHP files.
